...
#por que la maestra lo recomendo
...
m = 2
opr = np.zeros(shape=(numclusters, 
x.T.shape[0], x.T.shape[1]))
v = np.zeros(shape=(numclusters, 
x.T.shape[0]))
...
print(U)
U = np.around(U)
clusters = np.zeros(shape=(numclusters, 
x.T.shape[0], x.T.shape[1]))
clustersfre = []
for clus in range(numclusters):
...
ax.scatter3D(clustersfre[0][0], clustersfre[0][1], clustersfre[0][2], color='c')
ax.scatter3D(clustersfre[1][0], clustersfre[1][1], clustersfre[1][2], color='c')
ax.scatter3D(clustersfre[2][0], clustersfre[2][1], clustersfre[2][2], color='c')
plt.title("Muestras")
